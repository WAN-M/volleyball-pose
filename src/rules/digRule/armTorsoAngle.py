...
def detect(image, candidate, subset)->bool:
    flag = True
    # 判断左臂与躯干，选取5,6,11
    if not judge_angle(candidate[subset[0][5]][0:2],
                       candidate[subset[0][6]][0:2],
                       candidate[subset[0][11]][0:2]):
        flag = False
    # 判断右臂与躯干，选取2,3,8
    if not judge_angle(candidate[subset[0][2]][0:2],
                       candidate[subset[0][3]][0:2],
                       candidate[subset[0][8]][0:2]):
        flag = False
    return flag